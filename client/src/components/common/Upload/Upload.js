...
const Upload = ({ imageUrls, handleNewImage }) => {
  ...
  const removeImgUrl = item => {
    ...
    setDeletedImagesURLs([...deletedImagesURLs, deletedImgUrl]);
  };
  ...
  const showImages = e => {
    ...
    setAddedImagesFiles([...filesFromInput]);
    const newImgs = [];
        ...
        <Box style={{ margin: '8px 0 8px' }}>
          {images.map((item, index) => {
            return(<div key={item + index} className={classes.imgBox}>
              ...
              <IconButton style={{position: "absolute", bottom: 0, right: 0}} onClick={() => removeImgUrl(index)}><CloseIcon /></IconButton>
            </div>) ;
          })}
          {blobFiles.map((item , index) => {
            return  (<div key={item + index} className={classes.imgBox}>
              <img src={item} alt={item} className={classes.img}/>
              <IconButton style={{position: "absolute", bottom: 0, right: 0}} onClick={() => removeNewFile(index)}><CloseIcon /></IconButton>
            </div>)
          })}
        </Box>
      )}
